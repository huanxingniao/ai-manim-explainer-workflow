...
生成文件后立即验证语法：
```bash
source venv/bin/activate && python -c "import py_compile; py_compile.compile('{filename}.py', doraise=True)" && echo "SYNTAX OK"
```
...
// turbo
```bash
source venv/bin/activate && python /tmp/gen_tts_{topic}.py
```
...
```bash
source venv/bin/activate && pip install openai-whisper  # 首次需要
...
// turbo
```bash
source venv/bin/activate && manimgl {filename}.py {ClassName} -w 2>&1
```
...
## 当前项目状态
- 主 Manim 文件：`/Users/xuhongzhe/工程/测试项目/Manim/attention_v4.py`
- 句级节拍时长：`/Users/xuhongzhe/工程/测试项目/Manim/timing.json`
- 词级触发事件：`/Users/xuhongzhe/工程/测试项目/Manim/events.json`（Whisper 已对齐，28个触发点）
